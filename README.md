...
Compose 显式生成镜像 `xiaodu-llm-assistant:latest`，DSM UI 里可以按这个名字找到旧镜像。`docker-compose.yml` 里的 `build.pull: true` 会在构建时重新拉取 Dockerfile 的基础镜像；这不等同于 `docker compose build --no-cache`，如果 DSM 仍复用旧应用镜像，需要在 Container Manager 里删除 `xiaodu-llm-assistant:latest` 后再重建 Project。
...
建议开启缺省意图 `ai.dueros.common.default_intent`，并把回复方式设置为“服务配置满足”。这样用户表达没有命中明确意图时，平台也会把原始文本通过 `request.query.original` / `request.query.rewritten` 发给本服务。

如果另建自由文本意图，例如 `AskLLMIntent`，可以添加 `query`、`text` 或 `content` 槽位承接用户原始问题。本服务提取 query 的优先级是 `slots.query.value`、`slots.text.value`、`slots.content.value`、所有 `slot.value` 拼接，最后回退到 `request.query.rewritten` / `request.query.original`。

...
Compose 显式生成镜像 `xiaodu-llm-assistant:latest`，并设置 `pull_policy: build`，让 Compose 启动 Project 时优先重新构建这个应用镜像，而不是复用本地已有的 `xiaodu-llm-assistant:latest`。服务容器内仍监听 3000，DSM 宿主机端口映射为 `3030:3000`。
